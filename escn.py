...
class eSCN(BaseModel):
    ...
    def __init__(
        self,
        in_dim: int = None,  # not used
        out_dim: int = None,  # not used
        num_targets: int = None,  # not used
        use_pbc: bool = True,
    ...
    def forward(self, data):
        ...
        atomic_numbers = data.atoms.long()
        ...
        energy = torch.zeros([len(data.natoms),self.out_dim], device=device)
        energy.index_add_(0, data.batch, node_energy.view(-1,self.out_dim))
...
class EnergyBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.fc3 = nn.Linear(self.num_channels, out_dim, bias=False)
    ...
    def forward(self, x_pt) -> torch.Tensor:
        ...
        x_pt = self.fc3(x_pt)
        x_pt = x_pt.view(-1, self.num_sphere_samples, self.out_dim)
